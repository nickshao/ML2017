...
def read_test(filename):
    ...
    return data
def feature_scaling():
    train_data = read_train(sys.argv[1])
    test_data = read_test(sys.argv[2])
...
def del_train():
    ...
    ori_data = read_train(sys.argv[1])
    final_data = np.empty([5652,12])
    y_data = np.empty([5652,1])
    for mon in range(12):
        for sample in range(471):
            for fea in range(1):
                final_data[mon*471+sample][fea*8:fea*8+8] = data[fea+9][mon*480+sample+1:mon*480+sample+9]
            #for fea in range(4,8):
            #    final_data[mon*471+sample][fea*8:fea*8+8] = data[fea+10][mon*480+sample+1:mon*480+sample+9]
            final_data[mon*471+sample][8:12] = data[18][mon*480+sample+5:mon*480+sample+9]
            y_data[mon*471+sample] = ori_data[9][mon*480+sample+9]
    return final_data, y_data
def del_test():
    ori_data = read_test(sys.argv[2])
    data = feature_scaling()
    test_data = np.empty([240,12])
    fak_test = np.empty([240,12])
    y = np.empty([240, 1])
    for sample in range(240):
        for fea in range(1):
            test_data[sample][fea*8:fea*8+8] = data[fea+9][9*sample+5761:9*sample+5769]
            fak_test[sample][fea*8:fea*8+8] = data[fea+9][9*sample+5760:9*sample+5768]        
        #for fea in range(4,8):
        ...
        fak_test[sample][8:12] = data[18][9*sample+5764:+9*sample+5768]
        test_data[sample][8:12] = data[18][9*sample+5765:9*sample+5769]
...
def train():
    ...
    fin = np.empty([5892,12], dtype=float)
...
def test():
    ...
    ANS = [['id','value']]
    bias = 0.0
    weight = np.zeros([12,1], dtype=float)
